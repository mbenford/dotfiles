...
function PA:set_sink_volume(name, value)
	return self:set_volume("sink", name, value)
end
...
function PA:set_sink_mute(name, value)
	return self:set_mute("sink", name, value)
end

function PA:set_source_volume(name, value)
	return self:set_volume("source", name, value)
end

function PA:set_source_mute(name, value)
	return self:set_mute("source", name, value)
end
...
function PA:set_volume(type, name, value)
	local unmute_cmd = string.format("pactl set-%s-mute %s no", type, name)
	local set_volume_cmd = string.format("pactl set-%s-volume %s %s", type, name, value)
	return async
		.spawn(unmute_cmd)
		:next(function()
		...
		:next(function()
			return self:get_volume(type, name)
		end)
		:next(function(volume)
			if volume > 100 then
				return self:set_volume(type, name, "100%")
			end
			self:emit_signal(type .. "::volume", name, volume)
			return volume
		end)
end

function PA:set_mute(type, name, value)
	local cmd = string.format("pactl set-%s-mute %s %s", type, name, value)
	return async
		.spawn(cmd)
		:next(function()
			return self:get_mute(type, name)
		end)
		:next(function(mute)
			if mute then
				return 0
